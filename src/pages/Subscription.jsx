...
export default function Subscription() {
      ...
      try {
        ...
        plansData.forEach((p) => {
          ...
          if (firstKey) {
            ...
            // show monthly only if user already subscribed
            if (user?.currentPlan?.subscriptionId) {
              initialTypeSel[p._id] = "monthlyPrice";
            } else {
  ...
  const handlePricingTypeChange = (planId, type) =>
    setPricingTypeSelection((prev) => ({ ...prev, [planId]: type }));

  const handleSubscribe = (plan) => {
    const pricingKey =
      pricingSelection[plan._id] || getFirstPricingKey(plan.pricing);
    const type = pricingTypeSelection[plan._id] || "oneTimePrice";

    if (!isAuthenticated) {
      const checkoutRoute =
        type === "oneTimePrice" ? "/checkout-onetime" : "/checkout";
      navigate(
        `/login?redirect=${checkoutRoute}?planId=${plan._id}&pricingKey=${pricingKey}&pricingType=${type}`
      );
      return;
    }
    ...
    const checkoutRoute =
      type === "oneTimePrice" ? "/checkout-onetime" : "/checkout";
    navigate(
      `${checkoutRoute}?planId=${plan._id}&pricingKey=${pricingKey}&pricingType=${type}`
    );
  };
        ...
        <header className="mb-10">
          ...
          {targetServiceParam && (
            <div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-blue-800">
              You’re looking for:{" "}
              <span className="font-semibold">
                {servicesMap[targetServiceId]?.name || targetServiceParam}
              </span>
              {recommendedPlanId ? (
                <span className="ml-2">
                  — Recommended plan highlighted below.
                </span>
              ) : (
                <span className="ml-2">
                  — No plan explicitly lists this service.
                </span>
              )}
            </div>
          )}
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filteredPlans.map((plan) => {
            const selectedPricingKey =
              pricingSelection[plan._id] || getFirstPricingKey(plan.pricing);
            const selectedTier =
              selectedPricingKey && plan.pricing
                ? plan.pricing[selectedPricingKey]
                : null;
            const selectedType =
              pricingTypeSelection[plan._id] || "oneTimePrice";

            return (
              <div
                key={plan._id}
                className={`relative flex flex-col rounded-2xl border bg-white shadow-sm hover:shadow-md transition ${
                  recommendedPlanId === plan._id
              ...
              >
                <div className="p-6 border-b">
                  ...
                  <div className="mt-4 text-lg font-semibold">
                    {currency(selectedTier?.[selectedType])}
                  </div>
                </div>
                ...
                <div className="p-6 flex-1">
                  ...
                  >
                    <span className="text-sm font-medium text-gray-800">
                      What’s included ({plan.services?.length || 0})
                    </span>
                    <svg
                      className={`h-5 w-5 transition-transform ${
                        expanded[plan._id] ? "rotate-180" : ""
                      }`}
                      viewBox="0 0 20 20"
                      fill="currentColor"
                    >
                ...
                <div className="p-6 border-t">
                  <button
                    ...
                    className="w-full rounded-xl bg-blue-600 hover:bg-blue-700 text-white py-2 font-semibold transition"
                  >
                    {selectedType === "oneTimePrice" ? "Book Now" : "Buy Plan"}
                  </button>
                </div>
              </div>
            );
